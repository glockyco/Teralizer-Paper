...
Other assertions must be removed because they might validate methods
...
This systematic ablation study design allows us to isolate
...
execute only once per test, with their results reused for each generalization variant.
This architecture ensures fair comparison
while avoiding redundant computation—all variants work from identical specifications
...
The \textbf{\VariantNaive{}} variant adds random input generation with post-generation filtering
...
but are still enforced during filtering.
...
that it enables \ToolJqwik{}'s \texttt{Arbitraries}
