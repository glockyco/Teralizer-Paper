...
a barrier that limits adoption.
Our approach automates this transformation by extracting path-exact specifications
(specifications that precisely characterize behavior for specific execution paths)
directly from program semantics through single-path symbolic analysis.
The oracle problem fundamentally shapes this design (Section~\ref{sec:test-amplification}):
we can only generalize execution paths where developers have provided assertions
to validate expected behavior.
Other paths lack such validated oracles,
making it impossible to distinguish intended functionality from implementation artifacts.
This leads us to follow the concrete execution of existing tests,
collecting path conditions along the way without exploring alternative branches.
While this single-path approach also avoids the computational cost of full symbolic execution,
the oracle problem alone requires this focused strategy.
This white-box approach differs from JARVIS~\cite{peleg_2018_jarvis},
which uses black-box analysis of test input-output examples with predefined abstraction templates.
Because JARVIS analyzes only test code,
it produces overapproximations that may be too general for the actual execution paths.
In contrast, our analysis of both test and implementation enables
extraction of specifications that are exact for each covered path.
Beyond demonstrating feasibility, our comprehensive evaluation
characterizes which tests can be successfully generalized and identifies the barriers that prevent others,
providing empirical guidance for future research (Section~\ref{sec:limitations-eval}).
...
revealing that even under favorable conditions substantial filtering is required),
with comprehensive metrics captured at each stage to characterize
both successful transformations and failure modes.
...
\begin{itemize}
    ...
    generates property-based \ToolJqwik{} tests from extracted specifications,
...
and injects necessary dependencies including \ToolJqwik{}, \ToolPit{}, and \ToolJacoco{}.
...
and test generation approaches~\cite{tillmann_2008_pex,pacheco_2007_randoop},
...
within retry limits (typically when discard ratio exceeds 0.5).
