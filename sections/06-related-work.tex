...
extracting specifications directly from program semantics rather than relying on predefined templates~\cite{peleg_2018_jarvis}.
...
while \ToolTeralizer{} extracts path-exact conditions from existing tests
...
we extract path-exact conditions (e.g., \texttt{x >= 0}) directly from the implementation
...
All face similar challenges representing constraints for non-numeric types
in a form suitable for test generalization.
