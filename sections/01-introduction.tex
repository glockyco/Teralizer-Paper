...
testing only individual points within vast input spaces~\cite{orso_2014_software}.
...
a unit test verifying that \texttt{Math.abs(0)} returns \texttt{0} would 
...
then extracting specifications through single-path symbolic analysis
using Symbolic PathFinder's constraint collection mode~\cite{pasareanu_2013_symbolic},
...
showing how a simple equality assertion \texttt{Math.abs(0) = 0}
is transformed into a property \texttt{Math.abs(x) = x}
that holds for all non-negative values of x.
...
shows minimal improvement (0.05--0.07 percentage points)
