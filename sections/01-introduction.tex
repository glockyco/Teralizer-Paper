...
For example, a test might verify that \texttt{Math.max(3, 5)} returns \texttt{5},
...
For instance, our approach automatically transforms the test \texttt{assertEquals(5, Math.max(3, 5))}
into a property-based test that verifies \texttt{Math.max(a, b)} returns the larger value
for hundreds of generated pairs where \texttt{a <= b}.
...
achieving consistent mutation score improvements of 1--4 percentage points
