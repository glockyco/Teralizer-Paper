...
For example, as shown in Figure~\ref{fig:regression-detection},
a unit test which asserts that \texttt{abs(0)} returns \texttt{0} would 
still pass after an erroneous change from \texttt{x >= 0} to \texttt{x == 0}, 
whereas a property-based test which asserts \texttt{abs(x) = x} for non-negative \texttt{x} 
