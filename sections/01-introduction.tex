...
along with manual effort to define appropriate input generators and property specifications~\cite{goldstein_2024_pbt_practice}.
...
that validate behavior for entire input partitions.
...
Unlike prior work such as JARVIS that generalizes from examples using predefined abstraction templates~\cite{peleg_2018_jarvis},
...
with their existing developer-written tests expose the full complexity of practical applicability.
Beyond demonstrating feasibility, our implementation captures comprehensive metrics
...
Our evaluation reveals the current capabilities and boundaries of the semantics-based approach.
...
This systematic analysis identifies which technical advances would enable broader applicability:
...
when conditions align with current capabilities.
Unlike JARVIS which generalizes from examples using predefined abstraction templates
and produces over-approximations,
\ToolTeralizer{} extracts path-exact specifications directly from the implementation
through single-path symbolic analysis.
...
\item \textbf{Empirical mapping of applicability challenges:}
We systematically identify and quantify barriers to practical applicability,
