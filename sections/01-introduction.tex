...
but leave most inputs along the same execution path untested~\cite{orso_2014_software,ammann_2016_intro,myers_2011_art}.
...
still pass if the implementation incorrectly changes \texttt{x == 0} instead of \texttt{x >= 0}, 
...
requires manual effort to define both input generators and suitable properties,
...
to derive path-exact specifications through single-path symbolic analysis.
...
In contrast, our white-box approach leverages implementation analysis
...
\ToolTeralizer{} follows a three-phase pipeline:
(i) analyzing tests to identify tested methods and their assertions,
(ii) extracting specifications through single-path symbolic analysis~\cite{pasareanu_2013_symbolic}, and
(iii) generating property-based tests that explore additional inputs in the identified partitions.
...
we apply \ToolTeralizer{} to three complementary datasets that progressively
...
offer a middle ground between idealized and real-world scenarios.
...
This progression from favorable to challenging conditions highlights
the achievable fault-detection improvements
as well as the theoretical and practical limitations of the proposed approach.

Our evaluation shows modest yet consistent improvements under favorable conditions.
...
our analysis of 1,160 real-world projects reveals practical application barriers:
only 0.9\% of projects successfully complete the full generalization pipeline,
with failures primarily caused by 
assertion-to-method mapping issues, type support limitations, and constraint encoding challenges.
We classify these failures into those that can be resolved through additional engineering effort
...
and detailed filtering outcomes explaining why 99.1\% of real-world generalizations fail.
...
covering mutation score improvements, test suite size, and execution time impacts,
