...
some property-based tests do not detect any additional faults.
Thus, these tests increase test suite size and runtime
...
and retaining only those original and generalized tests
that strengthen the test suite's effectiveness.

\begin{table}[t]
  \caption{Mutation operators included in \ToolPit{}'s DEFAULTS group.}
...
Each row shows the name of the mutator,
a short description of its behavior,
and a source code representation
...
This ensures that every generalized test in the final test suite
...
An original test is removable if property-based tests were successfully created for all of its assertions.
For tests containing a single assertion, generalization ensures that
...
with the retained original tests.
