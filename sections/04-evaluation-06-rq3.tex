...
of the \VariantNaiveC{} and \VariantImprovedC{} test suite generalization variants
...
that cover the same input partition
with a single property-based test that covers the same partition instead.
For further discussion of this idea, see Section~\ref{sec:discussion}.

\subsubsection{Lines of Code in the Test Suite}
\label{sec:test-suite-line-count}

While test counts remain relatively stable through the test suite reduction mechanism,
...
from original test classes (as described in Section~\ref{sec:transformation-pipeline}).
...
Integration of generalized methods into original test classes would reduce duplication,
...
preventing unintended adverse effects on developer-written code
and mutation testing (as discussed in Section~\ref{sec:transformation-pipeline}).
...
\begin{figure}
  ...
  \caption{Runtime comparison between original test runtime and generalized test runtime. The runtime differences measure how much longer each generalized test takes to execute (per try) compared to the corresponding original test.}
  \Description{Two bar charts comparing runtime overhead of generalized tests.
  Left chart shows mean runtime difference in milliseconds between original and generalized tests.
  ...
  Right chart shows mean runtime difference per try in milliseconds.
