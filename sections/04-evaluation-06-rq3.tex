...
from original test classes (Section~\ref{sec:generalized-test-creation}).
...
% Test generalization adds 0--3 tests to final test suites after compensation,
% but increases lines of code by 5--59\% due to explicit constraint encoding and test isolation.
% Test suite runtime increases 1.5--3$\times$ for \ToolEvoSuite{}-generated tests
% and 9--57\% for developer tests,
% primarily from property-based testing overhead and the number of \tries{}.
% These impacts vary significantly based on test architecture:
% single-assertion tests enable near-complete compensation
% while multi-assertion tests resist consolidation.
% Implementation choices like constraint libraries and test integration
% could substantially reduce these overheads.
% \end{minipage}}
% \end{center}
