...
We therefore constructed a new evaluation dataset that combines projects from three complementary sources: the EqBench benchmark~\cite{TODO}
representing ideal conditions for specification extraction, utility methods extracted from 106 Apache Commons files to bridge
...
practical settings. This design enables us to evaluate both where the approach succeeds and, equally importantly, where
it encounters fundamental boundaries.

\subsubsection{EqBench}
\label{sec:eqbench}

The \DatasetEqBench{} benchmark~\cite{badihi_2021_eqbench} represents
an ideal evaluation target for automated generalization.
...
despite offering fewer remaining oppertunities for mutation score improvement through generalization.
...
This search yielded 247 classes from 106 source files,
...
Understanding where automated approaches fail proves as valuable as demonstrating where they succeed.
...
and follow standard directory structures (to enable large-scale automated processing without special-casing).
...
The \DatasetRepoReapers{} results are insightful precisely through the generalization failures they reveal.
...
We therefore reserve detailed analysis of \DatasetRepoReapers{} for RQ4,
