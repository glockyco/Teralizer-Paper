...
RQ5 established the causes of unsuccessful generalizations
under controlled conditions where projects were selected to align with \ToolTeralizer{}'s capabilities.
Real-world application, however, faces projects with testing practices and code patterns
that may introduce challenges beyond those observed in controlled settings.
This research question examines how \ToolTeralizer{} performs
on 632 real-world Java projects from the RepoReapers dataset
(Section~\ref{sec:experimental-framework}),
evaluating the practical applicability of the approach
and identifying which barriers prevent broader adoption.

Section~\ref{sec:project-level-exclusions-extended} examines project-level exclusions
that prevent entire projects from completing the generalization pipeline,
categorizing exclusion causes by stage and identifying whether they stem from
\ToolTeralizer{}'s internal design decisions or external tool limitations.
...
and comparing exclusion rates to controlled settings.
...
these lower-level exclusions cascade into project-level exclusions.
...
The pipeline exhibits a distinct funnel pattern with two major barriers
...
For assertion exclusions, all 255 projects result from filter rejections,
indicating these projects contain only assertion patterns
currently unsupported by \ToolTeralizer{}
...
and one internal failure occurs due to an exceeded specification extraction timeout
(60 seconds per test method).
%
...
exceeded timeouts (60 seconds per project), and external tool failures.
...
while remaining 4 projects are due to \ToolPit{} reports in non-standard locations.
...
already reach the configured timeout (60 seconds per test suite variant)
...
and 10.0\% encounter null pointer exceptions.
Only 18.3\% of failures are genuine assertion failures
where tests execute to completion but produce incorrect results.

The \texttt{TestType} filter increases from 0.8\% to 12.5\% rejection.
In the RepoReapers projects, this filter exclusively rejects tests without @Test annotations,
specifically those following JUnit 3's method naming convention
(method names starting with ``test'').
All 9,277 real-world rejections occur in 70 projects
that contain legacy JUnit 3 test methods,
with no @ParameterizedTest or other JUnit 5 annotation rejections observed.
...
from 5.5\% to 16.9\% due to the higher test-level exclusion rates.
