...
Test amplification exploits knowledge embedded in existing test cases
(e.g., meaningful input data and expected outputs encoded as assertions)
to automatically enhance manually written tests. Going beyond simply
maximizing coverage, amplification techniques target diverse engineering goals,
such as improving regression detection or refining fault localization~\cite{danglot_2019_snowballing}.
...
A central challenge in all test amplification approaches is the oracle problem:
...
such as \texttt{abs(x) == x} for non-negative \texttt{x}.
\emph{Shrinking} minimizes failing inputs to simplify debugging,
such as reducing a failing input from 776837 to 1.
...
and verifies that the property holds for each.
...
However, adoption of property-based testing remains limited in practice~\cite{goldstein_2024_pbt_practice}.
...
but validates it only for a single case.
...
A unit test verifying \texttt{abs(0) = 0}
