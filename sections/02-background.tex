...
For example, a test which asserts that \texttt{abs(0)} returns \texttt{0}
validates \texttt{abs} for only a single input-output pair,
missing regressions that preserve the behavior at that point
but violate the general property \texttt{abs(x) = x}
which should hold when \texttt{x >= 0}
