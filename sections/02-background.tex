...
\emph{Generators} produce inputs according to specified constraints, such as positive integers.
\emph{Properties} express invariants that must hold for all generated inputs, such as \texttt{abs(x) >= 0}.
\emph{Shrinking} algorithms minimize failing inputs to simplify debugging,
such as reducing a failing input from 2147483647 to 1 when the same property violation occurs.
...
PBT frameworks typically generate hundreds to thousands of test inputs
...
the path condition \texttt{x >= 0} (the branch condition) and the symbolic output \texttt{x} (the return value).
...
which can record exact conditions and solve them efficiently. Non-linear
arithmetic and floating-point operations are more problematic: tools can still
represent them precisely, but constraint solving quickly becomes computationally intractable,
often leading to timeouts~\cite{de_moura_2008_z3}. Strings,
arrays, and complex objects pose the greatest barrier: symbolic representations
...
we cannot create meaningful property-based tests without accurate models.
...
when property-based testing frameworks cannot efficiently produce inputs
satisfying complex constraints.
This input generation difficulty represents a fundamental computational challenge
that frameworks cannot fully overcome through filtering or constraint encoding~\cite{claessen_2000_quickcheck,link_2022_jqwik}.
Thus, while test generalization can theoretically handle any accurately modeled behavior,
practical success currently requires both precise specifications and tractable constraints.
