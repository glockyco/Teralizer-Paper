...
Danglot et~al.'s taxonomy~\cite{danglot_2019_snowballing} distinguishes four categories of amplification:
...
such as statement, branch, and path coverage~\cite{zhu_1997_software}
cannot discriminate between original and generalized tests,
because both traverse the same control-flow paths with different inputs.
Mutation testing provides this discrimination by measuring a test suite's ability to detect (kill)
artificially introduced faults~\cite{jia_2011_analysis}.
...
Common mutation operators include arithmetic replacement (\texttt{+} to \texttt{-}),
