...
and (5)~test suite reduction filters generalized tests to retain only those that measurably improve the mutation score of the overall test suite.
...
Despite the change, the test still passes because the tested inputs still follow the same execution paths.
However, boundary cases where \texttt{sales = 2 * target} now
