...
\newcommand{\VariantNaives}{\textsc{Naive\textsubscript{10/50/200}}}
...
\newcommand{\VariantImproveds}{\textsc{Improved\textsubscript{10/50/200}}}
...
\title[Teralizer: A Semantics-Based Test Generalization Approach]{Teralizer: Semantics-Based Test Generalization from Conventional Unit Tests to Property-Based Tests}
...
%% article.
\begin{abstract}
Conventional unit tests validate single input-output pairs, leaving most inputs of an execution path untested.
Property-based testing can address this shortcoming by generating multiple inputs satisfying properties.
However, property-based testing requires significant manual effort to define properties and their constraints.
We propose a semantics-based approach that automatically transforms
conventional unit tests into property-based tests by extracting specifications
from implementations via single-path symbolic analysis.
We demonstrate this approach through Teralizer, a prototype implementation for Java
that transforms JUnit tests into property-based jqwik tests.
Unlike prior work that generalizes from input-output examples,
Teralizer derives specifications from program semantics.

We applied Teralizer to three progressively challenging datasets to evaluate our approach.
On EvoSuite-generated tests for EqBench and Apache Commons utilities,
Teralizer improved mutation scores by 1--4 percentage points.
In contrast, generalization of mature developer-written tests from Apache Commons utilities
showed only 0.05--0.07 percentage points improvement.
A detailed analysis of the results from 632 real-world Java projects from RepoReapers
highlights several applicability barriers of Teralizer:
only 1.7\% of projects completed the generalization pipeline,
with the majority of failures caused by type support limitations in symbolic analysis and static analysis limitations in our prototype.
Based on the results, we provide a roadmap for future work, distinguishing addressable engineering limitations
from fundamental research challenges that need to be tackled to advance the field of test generalization.

Artifacts available at: \url{https://doi.org/10.5281/zenodo.17950381}
\end{abstract}
...
\input{sections/03-approach-00-intro}
\input{sections/03-approach-01-test-analysis}
\input{sections/03-approach-02-mut-identification}
\input{sections/03-approach-03-spec-extraction}
\input{sections/03-approach-04-generalization}
\input{sections/03-approach-05-reduction}

\input{sections/04-evaluation-01-intro}
\input{sections/04-evaluation-02-framework}
\input{sections/04-evaluation-04-rq1}
\input{sections/04-evaluation-05-rq2}
\input{sections/04-evaluation-06-rq3}
\input{sections/04-evaluation-07-rq4}
\input{sections/04-evaluation-08-rq5}
\input{sections/04-evaluation-09-rq6}
...
The research was supported by the Austrian ministries BMIMI, BMWET and the State of Upper Austria in the frame of the SCCH COMET competence center INTEGRATE (FFG 892418).
\end{acks}
