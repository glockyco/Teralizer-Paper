...
%% article.
\begin{abstract}
Unit tests validate single input-output pairs, leaving most inputs along the same execution path untested.
Property-based testing can address this by generating multiple inputs satisfying properties,
...
conventional unit tests into property-based tests by extracting path-exact specifications
directly from implementations via single-path symbolic analysis.
...
establishes both the potential and boundaries of semantics-based test generalization.
On \ToolEvoSuite{}-generated tests, mutation scores improve by 1--4 percentage points,
while mature developer-written tests show minimal gains (0.05--0.07 percentage points).
Analysis of 632 real-world Java projects quantifies applicability barriers:
only 0.9\% successfully complete the pipeline, with failures primarily caused by assertion-to-method mapping issues,
type support limitations, and constraint encoding challenges.
...
These results demonstrate feasibility under favorable conditions while identifying
current limitations, providing a foundation for future advances.
Replication package available at \url{[URL]}.
\end{abstract}
