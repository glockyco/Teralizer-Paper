...
\begin{enumerate}
  ...
  \item the overall execution time of the test suite (Section~\ref{sec:test-suite-execution-time}),
  \item the execution time of individual tests (Section~\ref{sec:test-execution-time}).
\end{enumerate}

For (1)--(3), we only present the results
...
The primary reason we avoided this in the current implementation
was to keep \ToolTeralizer{}'s changes as unintrusive and isolated
from the original test suite (and other generalized tests) as possible
