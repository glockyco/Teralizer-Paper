...
We demonstrate this approach through \ToolTeralizer{}, a prototype implementation for Java
that transforms JUnit tests into property-based \ToolJqwik{} tests.
Unlike prior work that generalizes from input-output examples,
we derive specifications from program semantics.

We applied \ToolTeralizer{} to three progressively challenging datasets to evaluate our approach.
On \ToolEvoSuite{}-generated tests for \DatasetEqBench{} and Apache Commons utilities, 
mutation scores improved by 1--4 percentage points.
Mature developer-written tests from Apache Commons utilities showed only 0.05--0.07 percentage points improvement.
Results for 632 real-world Java projects from RepoReapers highlight applicability barriers:
only 1.7\% completed the generalization pipeline,
with failures caused primarily by type support limitations of symbolic analysis
and static analysis limitations of our prototype.
We distinguish addressable engineering barriers from fundamental research challenges
and provide a comprehensive dataset that characterizes test generalizability patterns.

Artifacts available at \url{https://zenodo.org/records/XXXXXXX}.
\end{abstract}
